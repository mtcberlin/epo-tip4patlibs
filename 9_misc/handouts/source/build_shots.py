...
def build_04(stage: Path) -> tuple[str, int, int]:
    ...
    nb = json.loads((REPO / "2_patstat_explorer" /
...
<div class="wrap">
  ...
  <p class="sub">2_patstat_explorer/1_Applicant_consolidation_notebook.ipynb ·
...
def build_06(stage: Path) -> tuple[str, int, int]:
    ...
    root = REPO / "4_patentreports/antibiotic_resistance"
...
def build_08(stage: Path) -> tuple[str, int, int]:
    ...
    report = (REPO / "5_ipscore/4_tool/ipscore_valuation.html").read_text(encoding="utf-8")
