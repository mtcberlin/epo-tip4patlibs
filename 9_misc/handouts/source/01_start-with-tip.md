...
| | |
|---|---|
| Folder | `9_misc/legacy/startwithtip/` |
...
> an apology:
> **A subscription is only needed if you want to change things.** `install.sh` installs Claude
> Code, but the first `claude` command asks you to sign in with your own Anthropic account. For
> the course that step is not needed: every notebook ships executed, and you read results rather
> than generate them. Sign in when you want to alter a notebook or build your own analysis.
>
