...
> EP/PCT-only view:
> **Credentials needed for the second notebook.** `2_national-coverage.ipynb` queries the
> DPMA register over DPMAconnect, which is authenticated. Set `DPMA_USER` and `DPMA_PASS` in the
> environment before running it — in a TIP terminal, `export DPMA_USER=... DPMA_PASS=...`, or add
> them to `~/.bash_aliases` so they survive a restart. Never write them into a notebook. Without
> them the notebook stops at the first request with a clear message; **the first notebook,
> `1_regional-leads.ipynb`, needs nothing beyond PATSTAT** and carries this block on its own.
>
