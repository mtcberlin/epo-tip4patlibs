...
> and V5 are also in `5_ipscore_rebuild/REBUILD_PLAN.md` under *Open questions*, which is where
...
| **Working dir** | `5_ipscore_rebuild/` unless a task says otherwise |
...
**The decision it makes.** Right now `5_ipscore_rebuild/worked_example.json` holds an invented
...
**The corpus** is `4_patentreports/2_antibiotic_resistance_rebuild/1_dataset_and_search_strategy_output/dataset.xlsx`
...
families = pd.read_excel(
    "../4_patentreports/2_antibiotic_resistance_rebuild/"
...
Open `5_ipscore_rebuild/1_the_model.ipynb` and look at the two figures:
...
> from `5_ipscore_rebuild/`, and the `/files/` fallback survives the `/home/jovyan` symlink.
...
`5_ipscore_rebuild/` — but **the proxy branch has never executed**. Offline there is no
...
- ~~`5_ipscore_rebuild/1_the_model.ipynb`~~ — **not re-run**, so untouched. The charts were
...
- ~~`5_ipscore_rebuild/4_assemble_tool.ipynb`~~ — **not re-run**, so cell 19 carries no baked URL
  and nothing needed clearing.
- ~~`5_ipscore_rebuild/4_tool/`~~ — the report was **not** regenerated; it already had its
