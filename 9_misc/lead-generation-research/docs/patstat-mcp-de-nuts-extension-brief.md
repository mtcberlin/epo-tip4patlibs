...
1. **PATSTAT query** (BigQuery, `patstat-mtc.patstat`) — one row per national-only German company
   ...
   `epo-tip4patlibs/3_lead_generation/regional_leads_full_coverage_bq.ipynb` (Step 1), widened to
...
2. **Resolve** each `appln_nr` via DPMA `search/AKZ=` → PLZ. Reference implementation to vendor
   (stdlib, ~150 lines): `epo-tip4patlibs/3_lead_generation/dpma/fetch.py`
...
3. **PLZ → NUTS3** via the bundled Eurostat crosswalk
   `epo-tip4patlibs/3_lead_generation/dpma/data/pc2025_DE_NUTS-2024_v1.0.csv` (NUTS 2024, CC-BY-SA-4.0)
