# Provenance — `7_ipscore_demo/` (imported, Warsaw live demo)

> **This folder is imported material and is not the maintained lineage.**
>
> | | |
> |---|---|
> | **Upstream repo** | `rickypriore/patlib-sessions` (private) |
> | **Branch** | `warsaw-live-demo-merge` |
> | **Commit** | `2a434f02` — *"Merge warsaw-live-demo: fix event date"*, 2026-08-25 |
> | **Upstream path** | `8_ipscore_rebuild_v2/` |
> | **Imported** | 2026-09-16, verbatim, for the PATLIB Warsaw live demo |
>
> Riccardo's repository stays the canonical source for this folder, the same rule
> `5_patentreports/` follows.

## How this relates to `6_ipscore_rebuild/`

It is **not** a separate lineage. This folder is a fork of *our own*
`8_ipscore_rebuild/` taken at commit `caacba5` (2026-08-15, before the
2026-08-26 renumbering), with one feature added on top: an interactive
questionnaire.

What Riccardo added that we do not have:

| File | What it is |
|---|---|
| `0_questionnaire.ipynb` | Interactive form for the 40 answers — the new entry point |
| `0_questionnaire_tool.html` | The generated form |
| `tools/build_questionnaire_html.py` | Builder that renders the form |
| `TIP4PATLIBS_IPScore_NotebookLogic_Explained.pptx` | His deck for the demo |

What we have that this folder does not: `2_evidence_from_patstat_output/` —
the outputs of the evidence run on TIP (2026-08-15).

### Why it was imported beside module 6 rather than merged into it

`0_questionnaire.ipynb` writes `0_questionnaire_output/questionnaire.json`, and
this folder's `ipscore_kit.load_worked_example()` **prefers that file over the
shipped `worked_example.json`** for notebooks 2, 3 and 4. Merged into
`6_ipscore_rebuild/`, running the questionnaire would silently redirect the base
example that module 6's evidence layer is built to demonstrate. Keeping the two
apart means module 6 keeps behaving exactly as documented, and this folder
demonstrates the questionnaire as Riccardo built it.

**`6_ipscore_rebuild/` remains the maintained module.** Fixes belong there; this
folder is a snapshot of the demo as it was shown.

### Two things left as-imported, deliberately

- **Internal module numbers reflect the upstream layout.** The text and
  docstrings below still say "module 8" and point at `7_ipscore/`, which was the
  numbering before 2026-08-26. Nothing was rewritten, so the import stays a
  faithful copy and is easy to diff against upstream.
- **`TIP4PATLIBS_IPScore_NotebookLogic_Explained.pptx`** ships inside this folder
  and is a near-namesake of `9_documentation/course/TIP4PATLIBS_IPScore_NotebookLogic_Explained_con_note.pptx`.
  The one with `_con_note` carries the speaker notes and is the one the menu opens.

---

*Everything below is the upstream file as imported.*
