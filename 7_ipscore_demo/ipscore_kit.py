...
def load_answers(
    ...
    wherever a query decided or narrowed the score. If that file is there **and matches the
    patent :func:`load_worked_example` currently resolves to** (compared by
    ``docdb_family_id``), it wins.

    That match matters once notebook 0 is in the picture: it lets you switch to a different
    patent without re-running notebook 2, and this deliberately does not keep serving the
    previous patent's measured answers for the new one - it falls back and says so instead.

    Absent a match — the normal state until notebook 2 has been run on the current patent —
    this falls back to whatever :func:`load_worked_example` resolves to (notebook 0's
    interactive questionnaire if it has been saved, otherwise the shipped first pass), and
    the label says so. Reports must print the label: a valuation built on the first pass is
    a structured opinion, and the reader has to be able to tell the two apart at a glance.

    Returns ``(answers, source_label)``.
    """
    base = load_worked_example(example_path)
    path = Path(evidence_path)
    if path.exists():
        raw = json.loads(path.read_text(encoding="utf-8"))
        evidence_family = raw.get("patent", {}).get("docdb_family_id")
        current_family = base["patent"].get("docdb_family_id")
        ...
        return base["answers"], (
            f"the adviser's first pass - evidence_answers.json is for a different patent "
            f"(family {evidence_family}, not {current_family}) - run notebook 2 again for "
            f"this one")
    return base["answers"], "the adviser's first pass - notebook 2 has not been run"
