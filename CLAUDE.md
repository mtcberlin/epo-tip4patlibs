...
| `9_documentation/` | Working documents that are not themselves course modules. **TIP sessions:** `plan-tipsession.md` (✅ run 2026-08-15) · `results-tipsession.md` (O1, O2, V5 answered) · `plan-tipsession-2.md` (the evidence-layer run, ✅ done). **Teaching material:** `plan-course-material.md` and `course/` — a 45-min block per module (1, 3, 4, 5, 6, 8) with Lernziel/Teillernziele and the three phases Einleitung · Erarbeitung · Lernergebnis, plus a 3-slide workshop version of each. Both render from one Markdown source per module. `course/` holds the rendered A4 handouts; the Markdown, its YAML title-page sidecars and `build_handouts.py` live in `course/source/` |
