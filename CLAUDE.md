...
| `5_patentreports/` | Landscape reports (**Riccardo Priore**) — one folder, `antibiotic_resistance/`: corpus and search strategy, core analyses, advanced analyses, assembled report. Ships **pre-executed** (see below). The imported reference and the frozen MVP were removed 2026-09-16; recover with `git checkout 9d75a18 -- 5_patentreports` |
| `6_ipscore_rebuild/` | ✅ Complete — the course's own rebuild of the IPScore ideas as an explained notebook chain with a PATSTAT evidence layer, which measures 11 of the 40 answers (`2 measured · 6 informed · 32 judgement`). Starts at `0_questionnaire.ipynb`, Riccardo Priore's interactive form, imported 2026-09-16 from `rickypriore/patlib-sessions@2a434f02`: it writes `0_questionnaire_output/questionnaire.json`, which `load_worked_example()` then prefers over `worked_example.json` for notebooks 2–4, so a participant can value their own patent. Notebook 1 pins `kit.EXAMPLE_PATH` so it always reproduces the shipped case. Engine in `ipscore_kit.py`, model data in `ipscore_spec.json`, deliverable in `4_tool/`; `9_documentation/ipscore/` holds the EPO workbook the spec is extracted from |
| `9_documentation/` | Working documents that are not themselves course modules — see below |
| `9_documentation/legacy/` | Earlier worked end-to-end examples (Airbus filing strategy, TU Dortmund portfolio, Belgium regional distribution). Not shown in Warsaw |
| `9_documentation/lead-generation-research/` | DPMAconnect interface specs, NUTS mapping notes and implementation briefs behind module 4. Moved out of `4_lead_generation/` 2026-09-16. The `dpma/` package itself stayed in the module: `2_national-coverage.ipynb` imports it by walking **up** from the notebook, so it has to be an ancestor |
